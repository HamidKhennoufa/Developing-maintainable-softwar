...
from src.use_cases.employee_saver.dependencies.employee_saver_repository_interface import EmployeeSaverRepositoryInterface
...
def test_exception_response_on_repository_exception():
    ...
    employee_saver_use_case = EmployeeSaverUseCase(
        employee_saver_repository
    )
    actual_response = employee_saver_use_case.execute(employee)
    expected_response = ResponseDTO(
        executed=False,
...
def test_delegating_saving_to_repository():
    ...
    employee_saver_repository: EmployeeSaverRepositoryInterface =(
        MagicMock(spec=EmployeeSaverRepositoryInterface)
    )
...
def test_saving_right_object():
    employee_dto: EmployeeDTO = EmployeeDTO(
        first_name="first name",
        last_name="last name"
    )

    employee = Employee(first_name="first name", last_name="last name")
    employee_saver_repository: EmployeeSaverRepositoryInterface = (
        MagicMock(spec=EmployeeSaverRepositoryInterface)
    )
    employee_saver_repository.save = MagicMock()
